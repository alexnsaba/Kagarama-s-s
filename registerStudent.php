...
<ul>
 ...
 <ul>
  ...
  <div class="content-wrapper" style="margin-left: 0.5">
   ...
   <center> <h2>Enter the Student's details and click Register</h2> </center> 
                <div class="panel-body"> 

  <form method="post" action="registerStudent.php" enctype="multipart/form-data" name="registration" class="form-horizontal">              
...
<div class="col-sm-8">
<input type="text" name="name" id="name" class="form-control" placeholder="Name of the student" required="required" >
</div>
</div>
...
<div class="col-sm-8">
<select name ="class" value="class" class="form-control" required="required" >
...
<div class="col-sm-8">
<input type="text" name="username" id="username" class="form-control" placeholder="student's username" required="required" >
</div>
</div>
...
<div><br>
  <?php  
    ...
    $selUser = mysqli_query($con,"select * from student where username='$username'");
    $num= mysqli_num_rows($selUser);
	if($num>0){
		      echo"<h1 style='color:red'><i class='fa fa-close'></i> This username is already taken. please try a different one.</h1>";

	} else{
    $ins= mysqli_query($con,"insert into student(name,username,year,class,admissionNumber) values('$name','$username','$year','$class',' ')");	
    if($ins){
		$sel = mysqli_query($con," select studentId from student where username='$username'");
		$row = mysqli_fetch_array($sel);
		$stId = $row['studentId'];
		$admsNumber = $year."/".$class."/".$stId;
		mysqli_query($con,"update student set admissionNumber ='$admsNumber' where studentId = '$stId' ");
