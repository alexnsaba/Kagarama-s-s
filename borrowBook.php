...
<ul>
 ...
 <ul>
  ...
  <div class="content-wrapper" style="margin-left: 0.5">
   ...
   <center> <h2>Fill in the details and click lend Button</h2> </center> 
                <div class="panel-body"> 

  <form method="post" action="borrowBook.php" enctype="multipart/form-data" name="registration" class="form-horizontal">
                      
   <div class="form-group">
<label class="col-sm-2 control-label"> Select a book to Lend </label>
<div class="col-sm-8">
<select  name="cat" value="cat" id="cat" class="form-control"  required="required">
  <?php
  require_once'database.php';
  $selBook= mysqli_query($con,"select * from book where status='Available'");
  while($row=mysqli_fetch_array($selBook)){
    echo"<option>".$row['bookId']." ".$row['isbn']." ".$row['title']." edition ".$row['edition']." by ".$row['author']."</option>";
  }
 ...
 <div class="form-group">
<label class="col-sm-2 control-label"> Choose  User</label>
<div class="col-sm-8">
<select  name="cat" value="cat" id="cat" class="form-control"  required="required">
  <option>staff</option>
...
<div class="form-group">
<label class="col-sm-2 control-label"> Student / Staff Number  </label>
<div class="col-sm-8">
<input type="number" name="studno" id="expType" class="form-control" placeholder="student or staff Number eg 1" required="required" >
</div>
</div>
<div class="form-group">
...
<div class="col-sm-8">
<input type="number" name="bId" id="bId" class="form-control" placeholder="The Id of the book" required="required" >
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label"> Overdue Date </label>
<div class="col-sm-8">
<input type="date" name="dateOfReturn"  class="form-control"  required="required" >
</div>
</div>

<div class="col-sm-6 col-sm-offset-4">
<input type="reset" value="Cancel" class="btn btn-primary">
<input type="submit" name="lend" Value="Lend" class="btn btn-primary">
</div>
</form>
<div><br>
  <?php  
   ...
   require_once'database.php';   
   $insLend = mysqli_query($con,"insert into checkOuts(borrowerCategory,borrowerId,dateOfLend,dateOfReturn,status,bookId,isbn) 
    values('$cat','$studno','$dateOfLend','$dateOfReturn','unReturned','$bId','$isbn')");
   if($insLend){
    echo"<h1 style='color:blue'><i class='fa fa-check'></i> The Book has been lent successfully</h1>";
    mysqli_query($con,"update book set status='Lent' where bookId='$bId'");
   }
   else{
     echo"<h1 style='color:red'><i class='fa fa-close'></i> Ooops ! Operation failed. Please try again</h1>";
   }
   mysqli_close($con);
  }
  ?>
